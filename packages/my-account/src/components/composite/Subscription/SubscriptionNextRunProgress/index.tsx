...
function getProgressMax(subscription: OrderSubscription) {
  const distance = formatDistanceStrict(new Date(subscription.next_run_at as string), new Date(subscription.starts_at as string), { unit: 'minute' })
  return parseInt(distance.split(' ')[0]) ?? 0
}

function getProgressValue(subscription: OrderSubscription) {
  const distance = formatDistanceStrict(new Date(subscription.next_run_at as string), new Date(), { unit: 'minute' })
  return parseInt(distance.split(' ')[0]) ?? 0
}
...
function getProgressTitle(subscription: OrderSubscription) {
  ...
  if (subscription.status === 'active') {
    const distance = formatDistanceToNowStrict(new Date(subscription.next_run_at as string))
...
function SubscriptionNextRunProgress({ subscription, variant }: Props): JSX.Element {  
  ...
  return (
    <>
      <ProgressTitle variant={variant}>{getProgressTitle(subscription)}</ProgressTitle>
      <progress
        className={'progress'}
        max={max}
        value={remaining}
